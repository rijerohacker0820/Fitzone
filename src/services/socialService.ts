...
export const searchFriends = async (name: string): Promise<FriendSearchResult[]> => {
    if (!name.trim()) return [];
    const response = await apiClient.get(`/friends/search?name=${encodeURIComponent(name)}`);
    return response.data;
};
...
export const acceptFriendRequest = async (friendshipId: string): Promise<any> => {
...
export const getFeed = async (page: number = 1, pageSize: number = 20): Promise<Post[]> => {
    const response = await apiClient.get(`/posts/feed?page=${page}&pageSize=${pageSize}`);
    return response.data;
};
