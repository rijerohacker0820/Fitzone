...
export default function ACWRChart({ data }: ACWRChartProps) {
    ...
    if (!data || data.length === 0) {
        return (
            <View style={{ padding: 16, backgroundColor: colors.card, borderRadius: 16, margin: 16, height: 180, justifyContent: 'center', alignItems: 'center' }}>
                <Text style={{ color: colors.textSecondary }}>No data available for ACWR.</Text>
            </View>
        );
    }
    ...
    const points = data.map((d, i) => {
        ...
        const y = chartHeight - padding - ((d.ratio - minValue) / (maxValue - minValue)) * (chartHeight - padding * 2);
        ...
        <View style={{ padding: 20, backgroundColor: colors.card, borderRadius: 24, marginBottom: 24, borderWidth: 1, borderColor: '#E2E8F0' }}>
            ...
            <View style={{ height: 180, alignItems: 'center', justifyContent: 'center' }}>
                <Svg height={chartHeight} width={chartWidth}>
                    {/* Grid lines */}
                    <Line x1={padding} y1={padding} x2={padding} y2={chartHeight - padding} stroke={colors.textSecondary} strokeWidth="1" opacity="0.2" />
                    <Line x1={padding} y1={chartHeight - padding} x2={chartWidth - padding} y2={chartHeight - padding} stroke={colors.textSecondary} strokeWidth="1" opacity="0.2" />
                ...
                {data.length === 0 && (
                    <Text style={{ position: 'absolute', color: colors.textSecondary, fontSize: 12, fontStyle: 'italic' }}>
