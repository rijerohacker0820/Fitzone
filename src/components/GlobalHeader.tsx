...
export default function GlobalHeader({ title, showProfile = true, onProfilePress, leftAction, rightAction }: GlobalHeaderProps) {
    ...
    return (
        <View style={[styles.container, { height: totalHeight }]}>
            {Platform.OS === 'web' ? (
                <View style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(10px)' } as any]} />
            ) : (
                <BlurView intensity={80} tint="light" style={StyleSheet.absoluteFill} />
            )}

            <View style={[styles.content, { marginTop: insets.top, height: headerHeight }]}>
                {leftAction ? (
