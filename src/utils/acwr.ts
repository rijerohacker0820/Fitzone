...
export const calculateVolume = (routine: WorkoutRoutine): number => {
    ...
    routine.exercises.forEach(exercise => {
        exercise.sets.forEach(set => {
            ...
            if ((set.status === 'completed' || set.status === 'partial') && set.weight > 0) {
...
export const calculateACWR = (logs: WorkoutRoutine[]): { acute: number; chronic: number; ratio: number; status: 'Optimal' | 'Fatigue Risk' | 'Recovery' } => {
    // Sort logs by date desc
    const sorted = [...logs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

    if (sorted.length === 0) return { acute: 0, chronic: 0, ratio: 0, status: 'Recovery' };
