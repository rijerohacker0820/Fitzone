...
import { Trophy, TrendingUp, TrendingDown, Minus, Target, BarChart3, Calendar } from 'lucide-react-native';
...
const SimpleBar = ({ label, percentage, color, value }: { label: string; percentage: number; color: string; value: string }) => (
    <View style={simpleBarStyles.row}>
        <Text style={[simpleBarStyles.label, { color }]} numberOfLines={1}>{label}</Text>
        <View style={simpleBarStyles.barBg}>
            <View style={[simpleBarStyles.barFill, { width: `${Math.min(percentage, 100)}%`, backgroundColor: color }]} />
        </View>
...
const simpleBarStyles = StyleSheet.create({
    ...
    barBg: { flex: 1, height: 10, backgroundColor: 'rgba(128,128,128,0.15)', borderRadius: 5, marginHorizontal: 8, overflow: 'hidden' },
        ...
        return (
            <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
                ...
                <View style={styles.loadingContainer}>
                    ...
                    <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Analizando datos...</Text>
                </View>
            </View>
        );
    }
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
            ...
            >
                ...
                {comparison && (
                    <View style={[styles.card, { backgroundColor: cardBg, borderColor: isDark ? 'transparent' : colors.border }]}>
                        ...
                        <View style={styles.compGrid}>
                            <View style={styles.compItem}>
                                ...
                                <View style={styles.changeRow}>
                                    ...
                                    <Text style={[styles.changeText, {
                                        color: (comparison?.workoutCountChange ?? 0) >= 0 ? '#059669' : '#DC2626',
                                    }]}>{formatChange(comparison?.workoutCountChange ?? 0)}</Text>
                                </View>
                            </View>
                            ...
                            <View style={styles.compItem}>
                                <Text style={[styles.compValue, { color: colors.text }]}>
                                    {Math.round((comparison?.currentPeriod?.totalVolume ?? 0) / 1000)}k
                                </Text>
                                <Text style={[styles.compLabel, { color: colors.textSecondary }]}>Vol. (kg)</Text>
                                <View style={styles.changeRow}>
                                    {getChangeIcon(comparison?.volumeChange ?? 0)}
                                    <Text style={[styles.changeText, {
                                        color: (comparison?.volumeChange ?? 0) >= 0 ? '#059669' : '#DC2626',
                                    }]}>{formatChange(comparison?.volumeChange ?? 0)}</Text>
                                </View>
                            </View>
                            ...
                            <View style={styles.compItem}>
                                <Text style={[styles.compValue, { color: colors.text }]}>
                                    {Math.round((comparison?.currentPeriod?.totalDurationSeconds ?? 0) / 3600)}h
                                </Text>
                                <Text style={[styles.compLabel, { color: colors.textSecondary }]}>Tiempo</Text>
                                <View style={styles.changeRow}>
                                    {getChangeIcon(comparison?.durationChange ?? 0)}
                                    <Text style={[styles.changeText, {
                                        color: (comparison?.durationChange ?? 0) >= 0 ? '#059669' : '#DC2626',
                                    }]}>{formatChange(comparison?.durationChange ?? 0)}</Text>
                                </View>
                            </View>
                        </View>
                    </View>
