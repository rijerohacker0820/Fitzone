...
import { Search, UserPlus, UserCheck, Clock, UserX, X } from 'lucide-react-native';
import { useTheme } from '../context/ThemeContext';
import { useUser } from '../context/UserContext';
import { useToast } from '../components/Toast';
import {
    getFriends, searchFriends, sendFriendRequest,
...
export default function FriendsScreen() {
    ...
    const renderSearchItem = ({ item }: { item: FriendSearchResult }) => {
        ...
        const isActionLoading = actionLoading === item.userId || actionLoading === item.userId + '_follow';

        return (
            <View style={[styles.userCard, { backgroundColor: colors.card, borderColor: isDark ? colors.card : colors.border }]}>
                <Image
                ...
                <View style={styles.userInfo}>
                    ...
                    <View style={styles.statRow}>
                        <Text style={[styles.streak, { color: colors.primary }]}>🔥 {item.streak}</Text>
                    </View>
                </View>
                <View style={styles.actions}>
                    ...
                    <TouchableOpacity
                        style={[styles.followBtn, {
                            backgroundColor: item.isFollowing ? 'transparent' : colors.primary + '20',
                    ...
                    {status === 'Accepted' ? (
                        ...
                        <View style={[styles.statusBadge, { backgroundColor: '#D9770620' }]}>
    ...
    const renderFriendItem = ({ item }: { item: FriendDto }) => (
        <View style={[styles.userCard, { backgroundColor: colors.card, borderColor: isDark ? colors.card : colors.border }]}>
            ...
            <View style={styles.userInfo}>
                <Text style={[styles.userName, { color: colors.text }]}>{item.fullName}</Text>
                <Text style={[styles.streak, { color: colors.primary }]}>🔥 {item.streak} días</Text>
            </View>
            <View style={[styles.statusBadge, { backgroundColor: '#05966920' }]}>
                <UserCheck size={14} color="#059669" />
                <Text style={[styles.statusText, { color: '#059669' }]}>Amigos</Text>
    ...
    const renderRequestItem = ({ item }: { item: FriendDto }) => {
        ...
        return (
            <View style={[styles.userCard, { backgroundColor: colors.card, borderColor: isDark ? colors.card : colors.border }]}>
                ...
                <View style={styles.userInfo}>
                    <Text style={[styles.userName, { color: colors.text }]}>{item.fullName}</Text>
                    <Text style={[styles.subText, { color: colors.textSecondary }]}>Quiere ser tu amigo</Text>
                </View>
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            {/* Search Bar */}
            <View style={[styles.searchBar, { backgroundColor: isDark ? '#1A1A2E' : '#F1F5F9' }]}>
                ...
                {searchQuery.length > 0 && (
                    <TouchableOpacity onPress={() => { setSearchQuery(''); setSearchResults([]); setTab('friends'); }}>
            ...
            {tab !== 'search' && (
                <View style={styles.tabRow}>
                    <TouchableOpacity
                        style={[styles.tab, tab === 'friends' && { borderBottomColor: colors.primary, borderBottomWidth: 2 }]}
                        onPress={() => setTab('friends')}
                    >
                        <Text style={[styles.tabText, { color: tab === 'friends' ? colors.primary : colors.textSecondary }]}>
                            Amigos ({acceptedFriends.length})
                        </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        style={[styles.tab, tab === 'requests' && { borderBottomColor: colors.primary, borderBottomWidth: 2 }]}
            ...
            {loading && tab !== 'search' ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            <Text style={styles.emptyEmoji}>👥</Text>
                            <Text style={[styles.emptyTitle2, { color: colors.text }]}>Sin amigos aún</Text>
                            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
                    ...
                    renderItem={({ item }) =>
                        item.isRequester ? (
                            <View style={[styles.userCard, { backgroundColor: colors.card, borderColor: isDark ? colors.card : colors.border }]}>
                                ...
                                <View style={styles.userInfo}>
                                    <Text style={[styles.userName, { color: colors.text }]}>{item.fullName}</Text>
                                    <Text style={[styles.subText, { color: colors.textSecondary }]}>Solicitud enviada</Text>
                                </View>
                                <View style={[styles.statusBadge, { backgroundColor: '#D9770620' }]}>
                                    <Clock size={14} color="#D97706" />
                                    <Text style={[styles.statusText, { color: '#D97706' }]}>Pendiente</Text>
