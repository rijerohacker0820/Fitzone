    ...
    return (
        ...
        >
            ...
            <View style={[styles.header, { paddingTop: insets.top + SPACING.sm }]}>
                ...
                <TouchableOpacity
                    style={[styles.postBtn, {
                        backgroundColor: canPost && !posting ? colors.primary : colors.border,
                    }]}
                    onPress={handlePost}
            ...
            <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
                ...
                {/* Selected workout */}
                {selectedWorkout && (
                    <View style={[styles.workoutPreview, { backgroundColor: isDark ? '#1A1A2E' : '#F0F4FF' }]}>
                        <View style={styles.workoutPreviewHeader}>
                            ...
                            <View style={{ flex: 1 }}>
                                ...
                                <Text style={[styles.workoutDetail, { color: colors.textSecondary }]}>
                                    {selectedWorkout.exercises.length} ejercicios · {Math.round(selectedWorkout.duration / 60)} min
                                </Text>
                            </View>
            ...
            <View style={[styles.toolbar, { borderTopColor: colors.border, backgroundColor: colors.card }]}>
                <TouchableOpacity style={styles.toolBtn} onPress={pickImage}>
                    ...
                    <Text style={[styles.toolLabel, { color: colors.primary }]}>Foto</Text>
                </TouchableOpacity>

                <TouchableOpacity style={styles.toolBtn} onPress={() => setShowWorkoutPicker(!showWorkoutPicker)}>
                    <Dumbbell size={22} color={colors.primary} />
                    <Text style={[styles.toolLabel, { color: colors.primary }]}>Entreno</Text>
                </TouchableOpacity>
            </View>

            {/* Workout picker */}
            {showWorkoutPicker && (
                <View style={[styles.workoutPicker, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
                    <Text style={[styles.pickerTitle, { color: colors.text }]}>Selecciona un entreno</Text>
                    <ScrollView style={{ maxHeight: 200 }}>
                        {workouts.length === 0 ? (
                            <Text style={[styles.noWorkouts, { color: colors.textSecondary }]}>
                            ...
                            workouts.map(w => (
                                <TouchableOpacity
                                    key={w.id}
                                    style={[styles.workoutOption, { borderBottomColor: colors.border }]}
                                    onPress={() => {
                                ...
                                >
                                    ...
                                    <Text style={[styles.workoutOptionDate, { color: colors.textSecondary }]}>
