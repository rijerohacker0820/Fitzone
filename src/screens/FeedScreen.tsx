...
import {
    ...
    StyleSheet, RefreshControl, Animated, ActivityIndicator, Platform,
    KeyboardAvoidingView, Modal, Dimensions, ImageBackground
} from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { Heart, MessageCircle, Share2, Send, X, Plus, ChevronRight } from 'lucide-react-native';
...
export default function FeedScreen() {
    ...
    const renderPost = ({ item }: { item: Post }) => {
        ...
        return (
            <Animated.View style={[styles.postCard, {
                ...
                {hasImage ? (
                    <ImageBackground source={{ uri: item.imageUrl || '' }} style={{ width: '100%', minHeight: 400, justifyContent: 'space-between' }}>
                        <LinearGradient colors={['rgba(0,0,0,0.5)', 'transparent', 'rgba(0,0,0,0.8)']} style={StyleSheet.absoluteFill} />
                        
                        {/* Header overlaid */}
                        <View style={styles.postHeader}>
                            <View style={styles.userRow}>
                                <Image source={{ uri: item.userAvatarUrl || 'https://i.pravatar.cc/150' }} style={[styles.avatar, { borderColor: BRAND.orange }]} />
                                <View style={styles.userInfo}>
                                    <Text style={[styles.userName, { color: '#FFF' }]}>{item.userFullName}</Text>
                                    <Text style={[styles.postTime, { color: 'rgba(255,255,255,0.7)' }]}>{formatTime(item.createdAt)}</Text>
                                </View>
                            </View>
                            {item.type !== 'Text' && (
                                <View style={[styles.typeBadge, { backgroundColor: 'rgba(255,255,255,0.2)' }]}>
                                    <Text style={[styles.typeBadgeText, { color: '#FFF' }]}>{item.type}</Text>
                                </View>
                            )}
                        </View>
                        ...
                        <View style={{ paddingBottom: SPACING.md }}>
                            {item.content ? (
                                <Text style={[styles.postContent, { color: '#FFF', textShadowColor: 'rgba(0,0,0,0.5)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 }]}>{item.content}</Text>
                            ) : null}

                            <View style={styles.actionsRow}>
                                <TouchableOpacity style={styles.actionBtn} onPress={() => handleLike(item)} activeOpacity={0.7}>
                                    <Heart size={26} color={item.isLikedByCurrentUser ? '#FF2E4D' : '#FFF'} fill={item.isLikedByCurrentUser ? '#FF2E4D' : 'none'} />
                                    <Text style={[styles.actionCount, { color: '#FFF' }]}>{item.likeCount}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={styles.actionBtn} onPress={() => setCommentModal(item)} activeOpacity={0.7}>
                                    <MessageCircle size={26} color="#FFF" />
                                    <Text style={[styles.actionCount, { color: '#FFF' }]}>{item.commentCount}</Text>
                                </TouchableOpacity>
                                ...
                                <View style={{ paddingHorizontal: SPACING.md }}>
                                    {(item?.recentComments ?? []).slice(0, 2).map(c => (
                                        <View key={c.id} style={styles.commentRow}>
                                            <Text style={[styles.commentAuthor, { color: '#FFF' }]}>{c.userFullName}</Text>
                                            <Text style={[styles.commentText, { color: 'rgba(255,255,255,0.8)' }]} numberOfLines={1}>{c.content}</Text>
                                        </View>
                                    ))}
                                    {item.commentCount > 2 && (
                                        <TouchableOpacity onPress={() => setCommentModal(item)}>
                                            <Text style={[styles.viewAllComments, { color: 'rgba(255,255,255,0.6)' }]}>
                    ...
                    <View>
                        ...
                        <View style={styles.postHeader}>
                            <View style={styles.userRow}>
                                <Image source={{ uri: item.userAvatarUrl || 'https://i.pravatar.cc/150' }} style={[styles.avatar, { borderColor: colors.primary }]} />
                                <View style={styles.userInfo}>
                                    <Text style={[styles.userName, { color: colors.text }]}>{item.userFullName}</Text>
                        ...
                        {item.workoutSummary ? (
                            <View style={[styles.workoutCard, { backgroundColor: isDark ? '#1A1A2E' : '#F0F4FF' }]}>
                                ...
                                <View style={styles.workoutStats}>
                                    ...
                                    <View style={[styles.workoutDivider, { backgroundColor: colors.border }]} />
                                    <View style={styles.workoutStat}>
                                        <Text style={[styles.workoutStatValue, { color: colors.primary }]}>{item?.workoutSummary?.totalSets ?? 0}</Text>
                                        <Text style={[styles.workoutStatLabel, { color: colors.textSecondary }]}>sets</Text>
                                    </View>
                                </View>
                            </View>
                        ) : null}
                        ...
                        <View style={styles.actionsRow}>
                            <TouchableOpacity style={styles.actionBtn} onPress={() => handleLike(item)} activeOpacity={0.7}>
                                <Heart size={22} color={item.isLikedByCurrentUser ? '#FF2E4D' : colors.textSecondary} fill={item.isLikedByCurrentUser ? '#FF2E4D' : 'none'} />
                                <Text style={[styles.actionCount, { color: item.isLikedByCurrentUser ? '#FF2E4D' : colors.textSecondary }]}>{item.likeCount}</Text>
                            </TouchableOpacity>
                        ...
                        {(item?.recentComments?.length ?? 0) > 0 && (
                            <View style={[styles.commentsPreview, { borderTopColor: colors.border }]}>
                                {(item?.recentComments ?? []).slice(0, 2).map(c => (
                                    <View key={c.id} style={styles.commentRow}>
                                        <Text style={[styles.commentAuthor, { color: colors.text }]}>{c.userFullName}</Text>
                                        <Text style={[styles.commentText, { color: colors.textSecondary }]} numberOfLines={2}>{c.content}</Text>
                                    </View>
                                ))}
                                {item.commentCount > 2 && (
                                    <TouchableOpacity onPress={() => setCommentModal(item)}>
    ...
    const renderEmpty = () => (
        <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptyTitle, { color: colors.text }]}>Tu feed está vacío</Text>
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            {loading && posts.length === 0 ? (
                <View style={styles.loadingContainer}>
                    ...
                    <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Cargando feed...</Text>
                </View>
            ) : (
                <FlatList
                    data={posts}
                ...
                >
                    <View style={[styles.commentSheet, { backgroundColor: colors.card }]}>
                        <View style={styles.commentSheetHeader}>
                            <Text style={[styles.commentSheetTitle, { color: colors.text }]}>Comentarios</Text>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item: c }) => (
                                <View style={styles.commentItem}>
                                    <Image
                                        source={{ uri: c.userAvatarUrl || 'https://i.pravatar.cc/150' }}
                                        style={styles.commentAvatar}
                                    />
                                    <View style={styles.commentContent}>
                                        <Text style={[styles.commentAuthorFull, { color: colors.text }]}>{c.userFullName}</Text>
                                        <Text style={[styles.commentBody, { color: colors.textSecondary }]}>{c.content}</Text>
                                        <Text style={[styles.commentTime, { color: colors.textSecondary }]}>{formatTime(c.createdAt)}</Text>
                                    </View>
                                </View>
                            )}
                        />
                        ...
                        <View style={[styles.commentInputRow, { borderTopColor: colors.border }]}>
                            <TextInput
                                style={[styles.commentInput, { color: colors.text, backgroundColor: isDark ? '#1A1A2E' : '#F1F5F9' }]}
